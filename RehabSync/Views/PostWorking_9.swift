...
/// 對應分頁、這張卡片才會被建立出來（見 `PostWorking9RetentionDetailSheet`），建立後才用時間範圍查這一組
            ...
            ScrollView {
                VStack(spacing: 20) {
                        ...
                        ForEach(exgTabs.indices, id: \.self) { index in
                            Button {
                                ...
                                Text(exgTabs[index].title)
                                    .font(.system(size: 20, weight: .semibold))
                                    .foregroundStyle(selectedExgChannel == index ? Color.white : PostWorking_9.mutedText)
                                    .padding(.horizontal, 16)
                                    .padding(.vertical, 8)
                                    .background(selectedExgChannel == index ? PostWorking_9.darkPurple : Color.clear)
                                    .clipShape(Capsule())
                            }
                            .buttonStyle(.plain)
                        }
                    ...
                    if let selectedExgChannel, exgTabs.indices.contains(selectedExgChannel) {
                        ...
                        PostWorking9ExgCard(title: tab.title, treatmentResultId: treatmentResultId, deviceId: tab.deviceId, channel: tab.channel, setStartTimeMs: setStartTimeMs, setEndTimeMs: setEndTimeMs)
                    }
                }
                .padding(28)
                .padding(.top, 60)
