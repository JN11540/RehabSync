...
struct PostWorking_22: View {
    ...
    /// （postworking22-realdata-plan.md §6.2）。值與 `PostWorking_2`／`PostWorking_9` 相同。
    ...
    /// 主要內容區塊之間的垂直間距。
    ///
    /// 🔴 **`PostWorking22StatsBlock` 的兩排間距必須用同一個常數。**
                ...
                GeometryReader { geo in
                    ScrollView(showsIndicators: false) {
                        VStack(alignment: .leading, spacing: Self.sectionSpacing) {
                            PostWorking22Header()
                            PostWorking22StatsBlock(treatmentResult: treatmentResult,
...
private struct PostWorking22StatRow: View {
    let treatmentResult: TreatmentResult
    /// 欄寬由 `PostWorking22StatsBlock` 統一計算後傳進來（§6.1）。
    let slot: CGFloat
...
private struct PostWorking22VasNoteRow: View {
    let treatmentResult: TreatmentResult
    /// 同 `PostWorking22StatRow.slot`，由 `PostWorking22StatsBlock` 統一給。
    let slot: CGFloat
    ...
    private var vasStat: PostWorking22Stat {
        PostWorking22Stat(icon: "bandage.fill", color: PostWorking_22.pink,
                         label: "疼痛評分", value: vasText,
                         change: "", isPositive: true, note: "")
    }

    private var noteStat: PostWorking22Stat {
        PostWorking22Stat(icon: "list.bullet.clipboard.fill", color: PostWorking_22.orange,
    ...
    var body: some View {
        HStack(spacing: 16) {
            PostWorking22StatCard(stat: vasStat)
                .frame(width: slot)
            // 橫跨第 2、3 欄：兩個 slot 再加上中間那道被吃掉的間距（§16.1）。
            PostWorking22StatCard(stat: noteStat)
...
private struct PostWorking22StatsBlock: View {
    let treatmentResult: TreatmentResult
    let onRequestReturn: () -> Void

    /// 兩排之間的距離。🔴 直接用 `PostWorking_22.sectionSpacing`，**不要另外宣告一個 24** ——
    /// 那樣它與外層 `VStack` 只是數字剛好一樣，改了外層這裡不會跟著變（§17.1）。
    private var totalHeight: CGFloat {
        PostWorking_22.statCardHeight * 2 + PostWorking_22.sectionSpacing
    }
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            HStack(spacing: 16) {
                VStack(spacing: PostWorking_22.sectionSpacing) {
                    PostWorking22StatRow(treatmentResult: treatmentResult, slot: slot)
                    PostWorking22VasNoteRow(treatmentResult: treatmentResult, slot: slot)
                }
...
private struct PostWorking22StatCard: View {
    ...
    var body: some View {
        ...
        .padding(16)
        .frame(maxWidth: .infinity, alignment: .leading)
        .frame(height: PostWorking_22.statCardHeight)
        .background(Color.white)
        .clipShape(RoundedRectangle(cornerRadius: 16))
        .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color.black.opacity(0.05)))
    }
